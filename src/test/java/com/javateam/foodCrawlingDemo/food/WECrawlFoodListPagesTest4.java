...
@SpringBootTest
@Slf4j
public class WECrawlFoodListPagesTest4 {
...
//        List<InfiniteFoodVO> foods = infiniteRepo.findByInfiniteFood(start, end);
        List<String> foods = infiniteRepo.findByInfiniteFood();
		log.info("식품 링크 = {}",foods);
		
		List<String> foodsImg = infiniteImgRepo.findByInfiniteFoodImg(start2, end2);
		log.info("식품 링크 = {}",foodsImg);
...
//					String foodImgId= foodListInfo.get(j+88) // <div class="inBox" >
...
//					log.info("식품 이미지 : " + foodImg);
					...
					log.info("식품 이미지 : " + foodSiteImg);
					
					
					// 이미지 확장자
					String saveImgFileNameExt = foodSiteImg.substring(foodSiteImg.lastIndexOf('.') + 1);
//						String productImgURL = "https:"+product.select("div[class='prodDetail-w'] img").attr("src").trim();
		            ...
		            InputStream in = new URL(foodSiteImg).openStream();
